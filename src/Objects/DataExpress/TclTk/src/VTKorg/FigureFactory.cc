...
void FigureFactory::MakeLine()
{
  ...
  while(1){
	...
	if(j!=1){
	  ...
	  actorMaker->SetColor(pdb->PDB->AtomName,pdb->PDB->ResidueSequenceNumber,pdb->PDB->ResidueName,pdb->PDB->TemperatureFactor);
	...
	actorMaker->SetColor(0.5, 0.5, 0.5);
...
void FigureFactory::MakeSphere()
{
  ...
  if(property->GetAtomMode()==AtomModeCA){
    ren->AddActor(CaOnly);
  }else{
...
void FigureFactory::MakePoint()
{
  ...
  while(1){
    if(pdbFileIsAtom(pdb)){
      ...
      //PointMaker->SetRadius(pdb->PDB->AtomName);
      pointMaker->SetPosition(x, y, z);
      pointMaker->MAKE();
      actorMaker->CreateActor();
      actorMaker->SetMapper(pointMaker->GetMapper());
      actorMaker->SetColor(pdb->PDB->AtomName,pdb->PDB->ResidueSequenceNumber,pdb->PDB->ResidueName,pdb->PDB->TemperatureFactor);
      AllMolecule->AddPart(actorMaker->GetActor());
      if(pdbFileIsCA(pdb))
	{
      ...
      dataStore->SET(pdb->PDB,actorMaker->GetActor(),pointMaker->GetMapper(),i);
