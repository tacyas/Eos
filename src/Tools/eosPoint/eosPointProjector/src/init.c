...
void
init0(eosPointProjectorInfo* info)
{
...
void
init1(eosPointProjectorInfo* info)
{
    char s[1024];
    int i;
    if(info->flagIn) {
...
khorosInit(int argc, char* argv[])
{
    ...
    sprintf(panepath, "%s/src/Tools/eosPoint/eosPointProjector/src/eosPointProjector.pane", eospath);
    khoros_initialize(argc, argv, "EOS");
    fpt = fopen(panepath, "r");    if(NULL!=fpt) {
        fclose(fpt);
        kclui_initialize(panepath, KGEN_NONE, "EOS", "eosPointProjector",
