...
void
init0(WeightCalculationOfCommonLineSearchInfo* info)
{
...
void
init1(WeightCalculationOfCommonLineSearchInfo* info)
{
    char s[1024];
    int i;
    if(!info->flagIn) {
    ...
    if(!info->flagLengthThresholdMode) {
        stringGetFromFile(s, "LengthThresholdMode", stdin, stdout, 0);
        info->LengthThresholdMode = stringGetNthIntegerData(s, 1, ", \t");
...
khorosInit(int argc, char* argv[])
{
    ...
    sprintf(panepath, "%s/src/Tools/rec3d/WeightCalculationOfCommonLineSearch/src/WeightCalculationOfCommonLineSearch.pane", eospath);
    khoros_initialize(argc, argv, "EOS");
    fpt = fopen(panepath, "r");    if(NULL!=fpt) {
        fclose(fpt);
        kclui_initialize(panepath, KGEN_NONE, "EOS", "WeightCalculationOfCommonLineSearch",
