...
void
init0(mrcImageOrientationSearchByHigherSelectionInfo* info)
{
...
void
init1(mrcImageOrientationSearchByHigherSelectionInfo* info)
{
    char s[1024];
    int i;
    if(!info->flagIn) {
...
khorosInit(int argc, char* argv[])
{
    ...
    sprintf(panepath, "%s/src/Tools/rec3d/mrcImageOrientationSearchByHigherSelection/src/mrcImageOrientationSearchByHigherSelection.pane", eospath);
    khoros_initialize(argc, argv, "EOS");
    fpt = fopen(panepath, "r");    if(NULL!=fpt) {
        fclose(fpt);
        kclui_initialize(panepath, KGEN_NONE, "EOS", "mrcImageOrientationSearchByHigherSelection",
