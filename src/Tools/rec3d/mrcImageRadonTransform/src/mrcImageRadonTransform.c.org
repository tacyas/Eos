...
lmrcImageRadonTransform(mrcImage* out, mrcImage* in, lmrcImageRadonTransformInfo linfo, int mode)
{
	...
	for(phi=0;   phi<out->HeaderN.z; phi++) {
		...
		for(srcz=0;srcz<in->HeaderN.z; srcz++){
		  ...
		  difference = x*cos(theta*out->HeaderLength.y*RADIAN)*sin(phi*out->HeaderLength.z*RADIAN)+y*cos(phi*out->HeaderLength.z*RADIAN)+z*sin(theta*out->HeaderLength.y*RADIAN)*sin(phi*out->HeaderLength.z*RADIAN)-(p-(out->HeaderN.x-1)/2)*out->HeaderLength.x;
