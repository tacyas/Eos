...
void
init0(ctfEnvelopFunctionInfo* info)
{
    ...
    info->Defocus = 100000;    info->flagDefocus = 0;
...
void
init1(ctfEnvelopFunctionInfo* info)
{
    char s[1024];
    int i;
    if(info->flagOut) {
...
khorosInit(int argc, char* argv[])
{
    ...
    sprintf(panepath, "%s/src/Tools/ctfInfo/ctfEnvelopFunction/src/ctfEnvelopFunction.pane", eospath);
    khoros_initialize(argc, argv, "EOS");
    fpt = fopen(panepath, "r");    if(NULL!=fpt) {
        fclose(fpt);
        kclui_initialize(panepath, KGEN_NONE, "EOS", "ctfEnvelopFunction",
