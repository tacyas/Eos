...
void
init0(ctfMultiFunctionInfo* info)
{
...
void
init1(ctfMultiFunctionInfo* info)
{
    char s[1024];
    int i;
    if(info->flagIn) {
...
khorosInit(int argc, char* argv[])
{
    ...
    sprintf(panepath, "%s/src/Tools/ctfInfo/ctfMultiFunction/src/ctfMultiFunction.pane", eospath);
    khoros_initialize(argc, argv, "EOS");
    fpt = fopen(panepath, "r");    if(NULL!=fpt) {
        fclose(fpt);
        kclui_initialize(panepath, KGEN_NONE, "EOS", "ctfMultiFunction",
