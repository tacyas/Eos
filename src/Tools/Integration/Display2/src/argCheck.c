...
void
argCheck(Display2Info* info, int argc, char* argv[])
{
    long i;
    char s[1024];
    FILE* fpt;
    ...
    for(i=1; i<argc; i++) {
        if(OPTION_FLAG==argv[i][OPTION_FLAG_POS]) {
            SSWITCH(argv[i]+OPTION_POS)
                ...
                SCASE("def") {
                    if(i+1<argc) {
                        info->Default = stringGetNthWord(argv[i+1], 1, " ,");
                        i++;
                        info->flagDefault++;
                    } else {
                        usage(argv[0]);
                ...
                SCASE("display") {
                    if(i+1<argc) {
                        info->geometry = stringGetNthWord(argv[i+1], 1, " ,");
                        i++;
                        info->flaggeometry++;
                    } else {
                        usage(argv[0]);
                ...
                SCASE("geometry") {
                    if(i+1<argc) {
                        info->display = stringGetNthWord(argv[i+1], 1, " ,");
                        i++;
                        info->flagdisplay++;
                    } else {
                        usage(argv[0]);
