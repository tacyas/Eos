...
void
argCheck(mrcRefImagexaFitInfo* info, int argc, char* argv[])
{
    long i;
    char s[1024];
    FILE* fpt;
    ...
    for(i=1; i<argc; i++) {
        if(OPTION_FLAG==argv[i][OPTION_FLAG_POS]) {
            SSWITCH(argv[i]+OPTION_POS)
                ...
                SCASE("ix") {
                    if(i+1<argc) {
                        info->ix = stringGetNthIntegerData(argv[i+1], 1, " ,");
                        i++;
                        info->flagix++;
                    } else {
                        usage(argv[0]);
                ...
                SCASE("xrange") {
                    if(i+1<argc) {
                        info->xrange = stringGetNthIntegerData(argv[i+1], 1, " ,");
                        i++;
                        info->flagxrange++;
                    } else {
                        usage(argv[0]);
                ...
                SCASE("arange") {
                    if(i+1<argc) {
                        info->arange = stringGetNthIntegerData(argv[i+1], 1, " ,");
                        i++;
                        info->flagarange++;
                    } else {
                        usage(argv[0]);
