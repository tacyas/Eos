...
void
init0(mrcImage1dAverageByCorrelationInfo* info)
{
...
void
init1(mrcImage1dAverageByCorrelationInfo* info)
{
    char s[1024];
    int i;
    if(!info->flagIn) {
...
khorosInit(int argc, char* argv[])
{
    ...
    sprintf(panepath, "%s/src/Tools/mrcImage/mrcImage1dAverageByCorrelation/src/mrcImage1dAverageByCorrelation.pane", eospath);
    khoros_initialize(argc, argv, "EOS");
    fpt = fopen(panepath, "r");    if(NULL!=fpt) {
        fclose(fpt);
        kclui_initialize(panepath, KGEN_NONE, "EOS", "mrcImage1dAverageByCorrelation",
