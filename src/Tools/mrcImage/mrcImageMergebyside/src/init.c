...
void
init0(mrcImageMergebysideInfo* info)
{
    info->fptIn1 = NULL;    info->flagIn1 = 0;
    info->fptIn2 = NULL;    info->flagIn2 = 0;
    info->fptOut = NULL;    info->flagOut = 0;
    info->fptconfigFile = NULL;    info->flagconfigFile = 0;
    info->sep = 0;    info->flagsep = 0;
    info->mode = 0;    info->flagmode = 0;
}

void
init1(mrcImageMergebysideInfo* info)
{
    char s[1024];
    int i;
    if(!info->flagIn1) {
...
khorosInit(int argc, char* argv[])
{
    ...
    sprintf(panepath, "%s/src/Tools/mrcImage/mrcImageMergebyside/src/mrcImageMergebyside.pane", eospath);
    khoros_initialize(argc, argv, "EOS");
    fpt = fopen(panepath, "r");    if(NULL!=fpt) {
        fclose(fpt);
        kclui_initialize(panepath, KGEN_NONE, "EOS", "mrcImageMergebyside",
