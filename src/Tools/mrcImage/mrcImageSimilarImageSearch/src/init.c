...
void
init0(mrcImageSimilarImageSearchInfo* info)
{
...
void
init1(mrcImageSimilarImageSearchInfo* info)
{
    char s[1024];
    int i;
    if(!info->flagIn) {
...
khorosInit(int argc, char* argv[])
{
    ...
    sprintf(panepath, "%s/src/Tools/mrcImage/mrcImageSimilarImageSearch/src/mrcImageSimilarImageSearch.pane", eospath);
    khoros_initialize(argc, argv, "EOS");
    fpt = fopen(panepath, "r");    if(NULL!=fpt) {
        fclose(fpt);
        kclui_initialize(panepath, KGEN_NONE, "EOS", "mrcImageSimilarImageSearch",
