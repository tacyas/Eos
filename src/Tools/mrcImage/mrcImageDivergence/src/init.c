...
void
init0(mrcImageDivergenceInfo* info)
{
...
void
init1(mrcImageDivergenceInfo* info)
{
    char s[1024];
    int i;
    if(info->flagIn) {
...
khorosInit(int argc, char* argv[])
{
    ...
    sprintf(panepath, "%s/src/Tools/mrcImage/mrcImageDivergence/src/mrcImageDivergence.pane", eospath);
    khoros_initialize(argc, argv, "EOS");
    fpt = fopen(panepath, "r");    if(NULL!=fpt) {
        fclose(fpt);
        kclui_initialize(panepath, KGEN_NONE, "EOS", "mrcImageDivergence",
