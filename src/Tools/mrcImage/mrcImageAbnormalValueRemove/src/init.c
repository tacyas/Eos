...
void
init0(mrcImageAbnormalValueRemoveInfo* info)
{
    ...
    info->Pattern = 0;    info->flagPattern = 0;
    info->mode = 0;    info->flagmode = 0;
}

void
init1(mrcImageAbnormalValueRemoveInfo* info)
{
    char s[1024];
    int i;
    if(!info->flagIn) {
...
khorosInit(int argc, char* argv[])
{
    ...
    sprintf(panepath, "%s/src/Tools/mrcImage/mrcImageAbnormalValueRemove/src/mrcImageAbnormalValueRemove.pane", eospath);
    khoros_initialize(argc, argv, "EOS");
    fpt = fopen(panepath, "r");    if(NULL!=fpt) {
        fclose(fpt);
        kclui_initialize(panepath, KGEN_NONE, "EOS", "mrcImageAbnormalValueRemove",
