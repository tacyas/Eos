...
fprintf(stdout,"2:Mode1+Output dz,dphi(pixel) to standard output for every peak. \n"); 
fprintf(stdout,"\n");
...
/* begin */
  ...
  if (mode==0){
    ...
    lmrcMakepwzCortemplate(&template ,tN ,dp ,dz ,&header ,in ,0);
    lmrcpwzTemplateCorrelation(&tmp,&in2,in,&template,header.sy ,header.ey,1);
	...
	i++;
      ...
      if ( ( maxdx = (double *)malloc(sizeof(double)*tN) ) == NULL ){
	fprintf(stderr ,"Enough memoly is not available.");
	exit(EXIT_FAILURE);
      }
      if ( ( maxdy = (double *)malloc(sizeof(double)*tN) ) == NULL ){
	fprintf(stderr ,"Enough memoly is not available.");
	exit(EXIT_FAILURE);
      }
      if ( ( maxxorg = (double *)malloc(sizeof(double)*tN) ) == NULL ){
	fprintf(stderr ,"Enough memoly is not available.");
	exit(EXIT_FAILURE);
      }
      if ( ( maxyorg = (double *)malloc(sizeof(double)*tN) ) == NULL ){
	fprintf(stderr ,"Enough memoly is not available.");
	exit(EXIT_FAILURE);
      }
      n=0;
	...
	i++;
      ...
      lmrcRefImagepwzCorPeak2to1(maxx,maxy,maxdata,refmaxx2,refmaxy2,refmaxdata,dN,N,tN,0);
      lmrcRefImagepwzCorPeakDistance(maxdx,maxdy,maxx,maxy,N,tN,&template2.image,maxxorg,maxyorg);
