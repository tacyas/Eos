...
void
init0(mrcImageRandomParticlesImageCreateInfo* info)
{
    info->fptIn     = stdin;    info->fptInList = stdin;    info->flagIn = 0;
    info->fptOut = stdout;    info->flagOut = 0;
    info->Nx = 1024;    info->flagNx = 0;
    info->Ny = 1024;    info->flagNy = 0;
    info->Seed = 0;    info->flagSeed = 0;
...
void
init1(mrcImageRandomParticlesImageCreateInfo* info)
{
    char s[1024];
    int i;
    if(!info->flagIn) {
...
khorosInit(int argc, char* argv[])
{
    ...
    sprintf(panepath, "%s/src/Tools/mrcImage/mrcImageRandomParticlesImageCreate/src/mrcImageRandomParticlesImageCreate.pane", eospath);
    khoros_initialize(argc, argv, "EOS");
    fpt = fopen(panepath, "r");    if(NULL!=fpt) {
        fclose(fpt);
        kclui_initialize(panepath, KGEN_NONE, "EOS", "mrcImageRandomParticlesImageCreate",
