...
init0(mrcImageTTestInfo* info)
{
    ...
    info->fptOutT = NULL;    info->flagOutT = 0;
    info->fptconfigFile = NULL;    info->flagconfigFile = 0;
    info->mode = 0;    info->flagmode = 0;
}

void
init1(mrcImageTTestInfo* info)
{
    char s[1024];
    int i;
    if(info->flagIn) {
    ...
    if(info->flagOutT) {
        info->fptOutT = fileOpen(info->OutT, "w");
    }
...
khorosInit(int argc, char* argv[])
{
    ...
    sprintf(panepath, "%s/src/Tools/mrcImage/mrcImageTTest/src/mrcImageTTest.pane", eospath);
    khoros_initialize(argc, argv, "EOS");
    fpt = fopen(panepath, "r");    if(NULL!=fpt) {
        fclose(fpt);
        kclui_initialize(panepath, KGEN_NONE, "EOS", "mrcImageTTest",
