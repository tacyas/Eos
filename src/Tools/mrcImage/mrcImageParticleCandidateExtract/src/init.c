...
void
init0(mrcImageParticleCandidateExtractInfo* info)
{
...
void
init1(mrcImageParticleCandidateExtractInfo* info)
{
    char s[1024];
    int i;
    if(!info->flagIn) {
...
khorosInit(int argc, char* argv[])
{
    ...
    sprintf(panepath, "%s/src/Tools/mrcImage/mrcImageParticleCandidateExtract/src/mrcImageParticleCandidateExtract.pane", eospath);
    khoros_initialize(argc, argv, "EOS");
    fpt = fopen(panepath, "r");    if(NULL!=fpt) {
        fclose(fpt);
        kclui_initialize(panepath, KGEN_NONE, "EOS", "mrcImageParticleCandidateExtract",
