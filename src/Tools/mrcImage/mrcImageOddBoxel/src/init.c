...
void
init0(mrcImageOddBoxelInfo* info)
{
    ...
    info->Boxel = 0;    info->flagBoxel = 0;
    info->Density = 1.35;    info->flagDensity = 0;
    info->Inverse=0;
    info->Absolute=0;
    info->Th = 0.0;    info->flagTh = 0;
    info->fptconfigFile = NULL;    info->flagconfigFile = 0;
    info->mode = 0;    info->flagmode = 0;
}

void
init1(mrcImageOddBoxelInfo* info)
{
    char s[1024];
    int i;
    if(!info->flagIn) {
...
khorosInit(int argc, char* argv[])
{
    ...
    sprintf(panepath, "%s/src/Tools/mrcImage/mrcImageOddBoxel/src/mrcImageOddBoxel.pane", eospath);
    khoros_initialize(argc, argv, "EOS");
    fpt = fopen(panepath, "r");    if(NULL!=fpt) {
        fclose(fpt);
        kclui_initialize(panepath, KGEN_NONE, "EOS", "mrcImageOddBoxel",
