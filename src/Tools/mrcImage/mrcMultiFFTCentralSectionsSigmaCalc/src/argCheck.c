...
void
argCheck(mrcMultiFFTCentralSectionsSigmaCalcInfo* info, int argc, char* argv[])
{
    long i;
    char s[1024];
    FILE* fpt;
            ...
            SSWITCH(argv[i]+OPTION_POS)
                SCASE("i") {
                    if(i+2<argc) {
                        ...
                        i++;
                        info->flagIn++;
                        info->PathIn = stringGetNthWord(argv[i+1], 1, " ,");
                        i++;
                        info->flagPathIn++;
                    } else {
                        usage(argv[0]);
                ...
                SCASE("iL") {
                    if(i+2<argc) {
                        info->InLikelihoodList = stringGetNthWord(argv[i+1], 1, " ,");
                        i++;
                        info->flagInLikelihood++;
                        info->PathL = stringGetNthWord(argv[i+1], 1, " ,");
                        i++;
                        info->flagPathL++;
                    } else {
                        usage(argv[0]);
                ...
                SCASE("i3d") {
                    if(i+2<argc) {
                        info->In3DList = stringGetNthWord(argv[i+1], 1, " ,");
                        i++;
                        info->flagIn3D++;
                        info->Path3D = stringGetNthWord(argv[i+1], 1, " ,");
                        i++;
                        info->flagPath3D++;
                    } else {
                        usage(argv[0]);
