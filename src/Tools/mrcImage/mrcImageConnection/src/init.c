...
void
init0(mrcImageConnectionInfo* info)
{
...
void
init1(mrcImageConnectionInfo* info)
{
    char s[1024];
    int i;
    if(!info->flagIn1) {
...
khorosInit(int argc, char* argv[])
{
    ...
    sprintf(panepath, "%s/src/Tools/mrcImage/mrcImageConnection/src/mrcImageConnection.pane", eospath);
    khoros_initialize(argc, argv, "EOS");
    fpt = fopen(panepath, "r");    if(NULL!=fpt) {
        fclose(fpt);
        kclui_initialize(panepath, KGEN_NONE, "EOS", "mrcImageConnection",
