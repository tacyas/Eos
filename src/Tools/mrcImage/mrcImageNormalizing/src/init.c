...
void
init0(mrcImageNormalizingInfo* info)
{
...
void
init1(mrcImageNormalizingInfo* info)
{
    char s[1024];
    int i;
    if(!info->flagIn) {
    ...
    if(info->flagContourMin) {
    }
    
    if(info->flagContourMax) {
    }
...
khorosInit(int argc, char* argv[])
{
    ...
    sprintf(panepath, "%s/src/Tools/mrcImage/mrcImageNormalizing/src/mrcImageNormalizing.pane", eospath);
    khoros_initialize(argc, argv, "EOS");
    fpt = fopen(panepath, "r");    if(NULL!=fpt) {
        fclose(fpt);
        kclui_initialize(panepath, KGEN_NONE, "EOS", "mrcImageNormalizing",
