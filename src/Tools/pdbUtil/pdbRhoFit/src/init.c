...
init0(pdbRhoFitInfo* info)
{
    ...
    info->fptOutPar = stdout;    info->flagOutPar = 0;
...
init1(pdbRhoFitInfo* info)
{
    ...
    if(info->flagOutPar) {
        info->fptOutPar = fileOpen(info->OutPar, "w");
    }
...
khorosInit(int argc, char* argv[])
{
    ...
    sprintf(panepath, "%s/src/Tools/pdbUtil/pdbRhoFit/src/pdbRhoFit.pane", eospath);
    khoros_initialize(argc, argv, "EOS");
    fpt = fopen(panepath, "r");    if(NULL!=fpt) {
        fclose(fpt);
        kclui_initialize(panepath, KGEN_NONE, "EOS", "pdbRhoFit",
