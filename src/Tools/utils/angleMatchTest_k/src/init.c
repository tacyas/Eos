...
void
init1(angleMatchTest_kInfo* info)
{
    char s[1024];
    int i;
    if(!info->flagIn1) {
...
khorosInit(int argc, char* argv[])
{
    ...
    sprintf(panepath, "%s/src/Tools/utils/angleMatchTest_k/src/angleMatchTest_k.pane", eospath);
    khoros_initialize(argc, argv, "EOS");
    fpt = fopen(panepath, "r");    if(NULL!=fpt) {
        fclose(fpt);
        kclui_initialize(panepath, KGEN_NONE, "EOS", "angleMatchTest_k",
