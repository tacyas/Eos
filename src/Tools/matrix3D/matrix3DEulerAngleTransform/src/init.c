...
void
init0(matrix3DEulerAngleTransformInfo* info)
{
    info->fptIn     = NULL;    info->fptInList = NULL;    info->flagIn = 0;
    info->fptOut = NULL;    info->flagOut = 0;
    info->RotationalMode = stringGetNthWord("ZONS", 1, "\0");    info->flagRotationalMode = 0;
    info->RotMode = stringGetNthWord("ZONS", 1, "\0");    info->flagRotMode = 0;
...
void
init1(matrix3DEulerAngleTransformInfo* info)
{
    char s[1024];
    int i;
    if(!info->flagIn) {
...
khorosInit(int argc, char* argv[])
{
    ...
    sprintf(panepath, "%s/src/Tools/matrix3D/matrix3DEulerAngleTransform/src/matrix3DEulerAngleTransform.pane", eospath);
    khoros_initialize(argc, argv, "EOS");
    fpt = fopen(panepath, "r");    if(NULL!=fpt) {
        fclose(fpt);
        kclui_initialize(panepath, KGEN_NONE, "EOS", "matrix3DEulerAngleTransform",
