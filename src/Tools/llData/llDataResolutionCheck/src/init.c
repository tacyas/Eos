...
void
init0(llDataResolutionCheckInfo* info)
{
...
void
init1(llDataResolutionCheckInfo* info)
{
    char s[1024];
    int i;
    if(!info->flagIn) {
...
khorosInit(int argc, char* argv[])
{
    ...
    sprintf(panepath, "%s/src/Tools/llData/llDataResolutionCheck/src/llDataResolutionCheck.pane", eospath);
    khoros_initialize(argc, argv, "EOS");
    fpt = fopen(panepath, "r");    if(NULL!=fpt) {
        fclose(fpt);
        kclui_initialize(panepath, KGEN_NONE, "EOS", "llDataResolutionCheck",
